...
app.get("/:id", async (req,res) => {
    try {
        ...
        const result = await db.collection("shortUrls").find({shortUrl : shorturl }).project({_id: 0, shortUrl : 0, userKey : 0}).toArray();
        if ( result.length != 0) {
            //open(result[0].longUrl)
            res.redirect(result[0].longUrl);
            //res.sendStatus(200); 
        } else
            res.send("incorrect short url!!!");
        client.close();
    }catch (err) {
        console.log(err);
        res.sendStatus(500);
...
app.post("/listByKey/:key", async (req, res) => {
    try {
        const client = await mongoClient.connect(DB_URL);
        const db = client.db("shortURLs");
        const params = req.params.key ;

        const result = await db.collection("shortUrls").find({userKey : params}).project({_id : 0, userKey : 0}).toArray();
        if ( result.length != 0)
            res.send(result);
        else
            res.send("No Url's found with given key");
...
app.delete("/deleteurl", async (req, res) => {
    try {
        ...
        const result = await db.collection("shortUrls").findOneAndDelete({userKey : key, shortUrl : shorturl});

        if ( result.value )
            res.status(200).json( { "message" : "The url is deleted!!!"});
        else 
            res.status(500).json({ "message" : "Url not exist!!!"});
